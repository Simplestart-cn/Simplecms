...
class ProfileController extends MemberbaseController {
    ...
    public function edit_post() {
    	if(IS_POST){
    		$userid=sp_get_current_userid();
    		$_POST['id']=$userid;
    		if ($this->users_model->field('id,user_nicename')->create()) {
    ...
    public function mobile_post(){
        ...
        $uid=sp_get_current_userid();
        $mobile = $_POST['mobile'];
        $result = $this->users_model->where(array('mobile'=>$mobile))->count();
        if($result){
            $this->error("该手机号已注册！");
            return false;
        }
        ...
        $res = $this->users_model->where(array('id'=>$uid))->save(array('mobile'=>$mobile));
        if($res != false){
            $this->success('修改成功！');
        }else{
            $this->error('修改失败！');
