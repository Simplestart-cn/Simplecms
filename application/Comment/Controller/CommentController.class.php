...
class CommentController extends MemberbaseController{
	...
	function index(){
		...
		/*$comments_obj = M("Posts");//field(文章对应的userID,用户表的字段)->where("验证用户是否通过审核")
		$c_title=$comments_obj->field("id,post_title")->where("post_status=1")->select();
		$co_title=array();
		
		foreach ($c_title as $u){
			$co_title[$u['id']]=$u;
		}
		$this->assign("co_title",$co_title);*/
	}
