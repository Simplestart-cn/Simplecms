...
function sp_sql_mx($tag){
	//文章内页调用模型
		$term_id=$tag; //M("TermRelationships")->where("object_id=$tag")->getField('term_id');		
		$model_id=M("Terms")->where("term_id=$term_id")->getField('model_id');    
		$model=M("Model")->where("id=$model_id")->find();
		$model_a=json_decode($model['model_config'],true);
		
		foreach ($model_a['config'] as $key=>$value){
			$field=$value['field'];
			$mod=M("ModelRelationships")->where(array("model_object"=>$id,"model_field"=>$field))->getField('model_value');
			$this->assign("mx$field",$mod);
				}
		//END
...
function sp_sql_pages($tag){
	...
	$field = !empty($tag['field']) ? $tag['field'] : '*';
	$limit = !empty($tag['limit']) ? $tag['limit'] : '';
	$order = !empty($tag['order']) ? $tag['order'] : 'page_date';

	//根据参数生成查询条件
	$where['page_status'] = array('eq',1);
	$where['page_type'] = array('eq',2);
	...
	$pages=$pages_model->field($field)->where($where)->order($order)->limit($limit)->select();
	return $pages;
}
...
function sp_get_parent_term($term_id){
	$term_id=intval($term_id);
	$term_obj = M("Terms");
	$path=$term_obj->where("status=1 and term_id=$term_id")->getField('path');
	$path = explode('-',$path);
	return $path[1];
}
/**
