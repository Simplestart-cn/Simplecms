...
class SettingController extends AdminbaseController{
	...
	function site(){
		...
		$user_options=$this->options_model->where("option_name='user_options'")->getField("option_value");
		$tpls=sp_scan_dir(C("SP_TMPL_PATH")."*",GLOB_ONLYDIR);
		$noneed=array(".","..",".svn");
		$tpls=array_diff($tpls, $noneed);
		$this->assign("templates",$tpls);
		
		$adminstyles=sp_scan_dir(C("SP_ADMIN_TMPL_PATH")."*",GLOB_ONLYDIR);
		...
		$this->assign("user_options",json_decode($user_options,true));
	...
	function site_post(){
		if (IS_POST) {
			...
			$configs["SP_ADMIN_DEFAULT_THEME"]=$_POST['options']['site_adminstyle'];
