...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		if($ucenter_syn){
			if(!isset($_SESSION["user"])){
				if(!empty($_COOKIE['sp_auth'])  && $_COOKIE['sp_auth']!="logout"){
					$sp_auth=sp_authcode($_COOKIE['sp_auth'],"DECODE");
					$sp_auth=explode("\t", $sp_auth);
					$auth_username=$sp_auth[1];
					$users_model=M('Users');
	...
	protected function check_login(){
		if(!isset($_SESSION["user"])){
			$this->error(L('ACCESS_DENIED'),U("User/center/login"));
		}
	...
	protected function  check_user(){
		...
		if($user_status==2){
			$this->error(L('ACTIVATION_TIPS'));
		}
		
		if($user_status==0){
			session("user",null);//只有前台用户退出
			$this->error(L('FORBIDDEN_TIPS'),__ROOT__."/");
		}
	}
