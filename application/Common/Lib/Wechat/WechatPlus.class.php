...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::AUTH_URL . 'appid=' . $appid . '&secret=' . $appsecret);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                throw new WxErrorException($json['errmsg'],$json['errcode']);
              //  return false;
            }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '')
    {
        ...
        $uri="https://api.weixin.qq.com/cgi-bin/ticket/getticket?type=jsapi";
            $json = $this->http_execute($uri);
    ...
    {
            $json = $this->http_execute('https://api.weixin.qq.com/cgi-bin' . self::CALLBACKSERVER_GET_URL);
            return $json['ip_list'];
    }
    ...
    {
            $uri = 'https://api.weixin.qq.com/cgi-bin' . self::MENU_CREATE_URL;
            $this->http_execute($uri,self::json_encode($data));
            return true;
    }
    ...
    {
        $uri='https://api.weixin.qq.com/cgi-bin' . self::MENU_GET_URL;
            return $this->http_execute($uri);
    }
    ...
    {
            $uri='https://api.weixin.qq.com/cgi-bin' . self::MENU_DELETE_URL ;
            $this->http_execute($uri);
            return true;
    }
    ...
    {
            $uri='https://api.weixin.qq.com/cgi-bin' . self::MEDIA_UPLOAD_URL. '&type=' . $type ;
           return $this->http_execute($uri,$data, true);
    }
    ...
    public function getMedia($media_id, $is_video = false)
    {
      //  if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        $url_prefix = $is_video ? str_replace('https', 'http', self::API_URL_PREFIX) : self::API_URL_PREFIX;
            $result = $this->http_execute($url_prefix . self::MEDIA_GET_URL . '&media_id=' . $media_id);
            return $result;
    }
    ...
    public function uploadForeverMedia($data, $type, $is_video = false, $video_info = array())
    {
        ...
        if ($is_video) $data['description'] = self::json_encode($video_info);
            return $this->http_execute('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_UPLOAD_URL . '&type=' . $type, $data, true);
    }
    ...
    {
            $uri='https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_NEWS_UPLOAD_URL;
            $result = $this->http_execute($uri, self::json_encode($data));
            return $result;
    }
    ...
    public function updateForeverArticles($media_id, $data, $index = 0)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_NEWS_UPDATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
    ...
    public function getForeverMedia($media_id, $is_video = false)
    {
        ...
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        //$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_GET_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            if (is_string($result)) {
                $json = json_decode($result, true);
    ...
    public function delForeverMedia($media_id)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array('media_id' => $media_id);
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_DEL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getForeverList($type, $offset, $count)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_BATCHGET_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_FOREVER_COUNT_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MEDIA_UPLOADNEWS_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
            $uri ='https://api.weixin.qq.com/cgi-bin' . self::MASS_SEND_URL;
           return $this->http_execute($uri, self::json_encode($data));
    }
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MASS_SEND_GROUP_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MASS_DELETE_URL . 'access_token=' . $this->access_token, self::json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MASS_PREVIEW_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::MASS_QUERY_URL . 'access_token=' . $this->access_token, self::json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param int $type 0:临时二维码；1:永久二维码，场景id(此时expire参数无效)；2:永久二维码,场景字符串(此时expire参数无效)
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 604800)
    {
       ...
       $uri = 'https://api.weixin.qq.com/cgi-bin' . self::QRCODE_CREATE_URL ;
            return $this->http_execute($uri, self::json_encode($data));
    ...
    public function getShortUrl($long_url)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::SHORT_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::USER_GET_URL . 'access_token=' . $this->access_token . '&next_openid=' . $next_openid);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        $uri='https://api.weixin.qq.com/cgi-bin' . self::USER_INFO_URL .'openid=' . $openid;
        return $this->http_execute($uri);
    }
    ...
    public function updateUserRemark($openid, $remark)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::USER_UPDATEREMARK_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::GROUP_GET_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getUserGroup($openid)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::USER_GROUP_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function createGroup($name)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::GROUP_CREATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateGroup($groupid, $name)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::GROUP_UPDATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateGroupMembers($groupid, $openid)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::GROUP_MEMBER_UPDATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function batchUpdateGroupMembers($groupid, $openid_list)
    {
        ...
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::GROUP_MEMBER_BATCHUPDATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
            return $this->http_execute('https://api.weixin.qq.com/cgi-bin' . self::CUSTOM_SEND_URL , self::json_encode($data));
    }
    ...
    public function getOauthAccessToken()
    {
        ...
        if (!$code) return false;
            $uri =self::API_BASE_URL_PREFIX . self::OAUTH_TOKEN_URL . 'appid=' . $this->appid . '&secret=' . $this->appsecret . '&code=' . $code . '&grant_type=authorization_code';
            $json= $this->http_execute($uri);
    ...
    {
            $uri =self::API_BASE_URL_PREFIX . self::OAUTH_REFRESH_URL . 'appid=' . $this->appid . '&grant_type=refresh_token&refresh_token=' . $refresh_token;
            $json= $this->http_execute($uri);
            $this->user_token = $json['access_token'];
            return $json;
    }
    ...
    {
            $uri =self::API_BASE_URL_PREFIX . self::OAUTH_USERINFO_URL . 'access_token=' . $access_token . '&openid=' . $openid;
            $json= $this->http_execute($uri,false);
            return $json;
    }
    ...
    {
        $uri =self::API_BASE_URL_PREFIX . self::OAUTH_AUTH_URL  . 'openid=' . $openid;
        $result = $this->http_execute($uri);
    ...
    public function setTMIndustry($id1, $id2 = '')
    {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::TEMPLATE_SET_INDUSTRY_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function addTemplateMessage($tpl_id)
    {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::TEMPLATE_ADD_TPL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::TEMPLATE_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::CUSTOM_SERVICE_GET_RECORD . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::CUSTOM_SERVICE_GET_KFLIST . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin' . self::CUSTOM_SERVICE_GET_ONLINEKFLIST . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin' . self::SHAKEAROUND_MATERIAL_ADD . 'access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function http_execute($uri, $param=null, $post_file = false, $isReturnArray=true)
    {
        $retryBusyTimes = 0;
        $retryAccssTonkenErrorTimes = 0;
        $retrySleepMillis = 1000;
        $maxRetryTimes = 3;
        do {
            $accessToken = $this->checkAuth();
            $uriWithAccessToken = $uri;
            $uriWithAccessToken .= mb_strrpos($uri,'?') == strlen($uri)-1 ? "access_token=" . $accessToken : "&access_token=" . $accessToken;
            if($param==null){
                $result = $this->http_get($uriWithAccessToken);
            }else{
                $result = $this->http_post($uriWithAccessToken,$param,$post_file);
            }
            if($result){
                $json = json_decode($result, true);
                if($json['errcode']!=0){
                    if ($json['errcode'] == -1) {
                        $sleepMillis = $retrySleepMillis * (1 << $retryBusyTimes) * 100;
                        usleep($sleepMillis);
                        $retryBusyTimes++;
                    } else if($json['errcode']==42001 || $json['errcode']==40001) {
                        $sleepMillis = $retrySleepMillis * (1 << $retryAccssTonkenErrorTimes) * 100;
                        usleep($sleepMillis);
                        $retryAccssTonkenErrorTimes++;
                        //accessToken 强制过期,刷新token
                        $this->resetAuth();
                    }else{
                        throw new WxErrorException($json['errmsg'],$json['errcode']);
                    }
                }else{
                    return json_decode($result, $isReturnArray);
                }
            }else{
                //请求返回码不是200,继续重试
                $retryBusyTimes++;
            }
        } while ($retryBusyTimes < $maxRetryTimes && $retryAccssTonkenErrorTimes < $maxRetryTimes);
        throw new Exception("微信服务端异常，超出重试次数");
    }
}
