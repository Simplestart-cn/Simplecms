...
class Tree {
    ...
    function get_treeview_menu($myid,$effected_id='example', $str="<span class='file'>\$name</span>", $str2="<span class='folder'>\$name</span>", $showlevel = 0,  $ul_class="" ,$li_class="" , $style='filetree ', $currentlevel = 1, $recursion=FALSE, $dropdown='hasChild') {
	   ...
	   $placeholder = '<ul><li><span class="placeholder"></span></li></ul>';
    	...
    	foreach ($child as $id => $a) {
    		@extract($a);
            $drochild = $this->get_child($a['id']);
    		if ($showlevel > 0 && is_array($drochild)){
                $onchild = false;
                foreach ($drochild as $id => $b) {
                    if($b['href'] == $_SERVER['REQUEST_URI']){
                        $onchild = true;
                    }
                }
    			$onchild ? $floder_status = " class='$dropdown $li_class on" : $floder_status = " class='$dropdown $li_class";
                // print_r($drochild);
    		}else{
    			$floder_status = " class='$li_class";;
    		}
            if($a['href'] == $_SERVER['REQUEST_URI']){
                $floder_status.=" on'";
            }else{
                $floder_status.="'";
            }
