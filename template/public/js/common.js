...
function fileAjaxUploader (option) {
    var file,
        fd = new FormData(),
        xhr = new XMLHttpRequest(),
        loaded, tot, per, uploadUrl,input,inputName,
        uploadUrl = option.uploadUrl,
        callback = option.callback,
        uploading = option.uploading,
        beforeSend = option.beforeSend,
        inputName = option.name ? option.name : 'files',
...
//图片上传
function uploadImage(value_id,preview_id,callback){
    var btn = this;
    fileAjaxUploader({
        //上传文件接收地址
        uploadUrl: "index.php?g=Asset&m=Asset&a=uploadImage",
        name:'user_avatar',  //表单input的name属性,图片字段名
        ...
        callback: function (res){
            ...
            console.log(res);
            if(res.status){
                $('#'+value_id).val(res.file);
                $('#'+preview_id).attr('src',res.file);
            }else{
                alert(res.message)
            }
        },
...
function resetImage(value_id,preview_id){
    $('#'+value_id).val('');
    $('#'+preview_id).attr('src','public/images/thumb.png');
}


//文件上传
function uploadFile(value_id,callback){
    var btn = this;
    fileAjaxUploader({
        //上传文件接收地址
        uploadUrl: "index.php?g=Asset&m=Asset&a=uploadFile",
        name:'user_avatar',  //表单input的name属性,图片字段名
        //file为上传的文件信息，可在此处做文件检测、初始化进度条等动作
        beforeSend: function(file) {
        },
