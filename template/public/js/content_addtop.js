...
function change_images(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        ...
        str += "<li id='image" + ids + "' class='col-md-3 col-sm-6 col-xs-12'><input type='hidden' name='" + returnid + "_url[]' value='" + n + "' style='width:310px;' class='input image-url-input'> <img src='" + n + "'  ondblclick='image_priview(this.src);' title='双击查看' /><div class='img-info'><input type='text' name='" + returnid + "_alt[]' value='" + filename + "' style='width:60%;' class='input image-alt-input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:flashupload('replace_albums_images', '图片替换','image"+ids+"',replace_image,'10,gif|jpg|jpeg|png|bmp,0','','','')\">替换</a>  <a href=\"javascript:remove_div('image" + ids + "')\">移除</a></div> </li>";
    });
